...
void HMHeuristic::init_hm_table(htuple &t) {
    map<htuple, int>::iterator it;
    for (it = hm_table.begin(); it != hm_table.end(); it++) {
        pair<htuple, int> hm_ent = *it;
        htuple &tup = hm_ent.first;
...
void HMHeuristic::extend_tuple(htuple &t, const Operator &op) {
    map<htuple, int>::const_iterator it;
    for (it = hm_table.begin(); it != hm_table.end(); it++) {
        pair<htuple, int> hm_ent = *it;
        htuple &entry = hm_ent.first;
...
int HMHeuristic::eval(htuple &t) {
    vector<htuple> partial;
...
int HMHeuristic::update_hm_entry(htuple &t, int val) {
...
void HMHeuristic::generate_all_tuples(int var, int sz, htuple &base) {
    if (sz == 1) {
...
void HMHeuristic::generate_all_partial_tuple(htuple &base_tuple, htuple &t,
                                             int index, int sz, vector<htuple> &res) {
    if (sz == 1) {
...
int HMHeuristic::check_tuple_in_tuple(const htuple &tup, const htuple &big_tuple) {
