...
typedef vector<pair<int, int> > htuple;
...
protected:
    ...
    // h^m table
    map<htuple, int> hm_table;
    bool was_updated;

    void init_hm_table(htuple &t);
    void update_hm_table();
    int eval(htuple &t);
    int update_hm_entry(htuple &t, int val);
    void extend_tuple(htuple &t, const Operator &op);

    // some helper methods
    int check_tuple_in_tuple(const htuple &tup, const htuple &big_tuple);
    void state_to_tuple(const State &state, htuple &t) {
    ...
    void get_operator_pre(const Operator &op, htuple &t) {
        for (int i = 0; i < op.get_prevail().size(); i++)
    ...
    void get_operator_eff(const Operator &op, htuple &t) {
        for (int i = 0; i < op.get_pre_post().size(); i++)
    ...
    void generate_all_tuples(int var, int sz, htuple &base);

    void generate_all_partial_tuple(htuple &base_tuple, vector<htuple> &res) {
        htuple t;
        generate_all_partial_tuple(base_tuple, t, 0, m, res);
    }
    void generate_all_partial_tuple(htuple &base_tuple, htuple &t, int index, int sz, vector<htuple> &res);

    void dump_table() const {
        map<htuple, int>::const_iterator it;
        for (it = hm_table.begin(); it != hm_table.end(); it++) {
            pair<htuple, int> hm_ent = *it;
            cout << "h[";
            print_tuple(hm_ent.first);
            cout << "] = " << hm_ent.second << endl;
        }
    }
    void print_tuple(htuple &tup) const {
